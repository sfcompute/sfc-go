...
// Permissions - Inspect what the caller is allowed to do.
...
func newPermissions(rootSDK *SDK, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Permissions {
	return &Permissions{
		rootSDK:          rootSDK,
...
// CheckPermissionHandler - Check permissions
...
func (s *Permissions) CheckPermissionHandler(ctx context.Context, request components.CheckPermissionRequest, opts ...operations.Option) (*operations.CheckPermissionHandlerResponse, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/preview/v2/permissions/check")
	if err != nil {
	...
	res := &operations.CheckPermissionHandlerResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.PermissionCheckResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.PermissionCheckResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
